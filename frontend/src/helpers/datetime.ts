...
// 全局时区配置，从系统状态中读取
let globalTimezone: string | undefined = undefined;

/**
 * 设置全局时区
 * @param timezone IANA 时区字符串，如 "Asia/Shanghai", "America/New_York"
 */
...
/**
 * 获取当前使用的时区
 * @returns 时区字符串，如果未设置则返回 undefined（使用浏览器默认时区）
 */
...
export function getDateTimeString(t?: Date | number | string | any, locale = i18n.language): string {
  ...
  // 如果配置了时区，添加到选项中
  if (globalTimezone) {
    options.timeZone = globalTimezone;
  }

  return new Date(tsFromDate).toLocaleDateString(locale, options);
}
