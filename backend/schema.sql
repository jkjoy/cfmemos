...
CREATE TABLE IF NOT EXISTS users (
    ...
    row_status INTEGER NOT NULL DEFAULT 0  -- 0: NORMAL, 1: ARCHIVED
);
...
CREATE TABLE IF NOT EXISTS identity_providers (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL,  -- 显示名称，如 "Google", "GitHub"
    type TEXT NOT NULL,  -- OAuth2类型：google, github, gitlab, oidc等
    identifier_filter TEXT DEFAULT '',  -- 可选的email域名过滤（逗号分隔）
    config TEXT NOT NULL,  -- JSON配置：client_id, client_secret, auth_url, token_url等
    created_ts INTEGER NOT NULL DEFAULT (strftime('%s', 'now')),
    updated_ts INTEGER NOT NULL DEFAULT (strftime('%s', 'now'))
);
